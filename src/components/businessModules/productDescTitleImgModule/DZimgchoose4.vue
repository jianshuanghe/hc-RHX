<template>
  <div class="PDTIV1 dingzhi-base bg-fff" v-if="renderThis">
    ...
    <!-- 选择上传文件bridge -->
    <textarea :class="{ 'text': !disabled, 'dingzhi-head': true}" maxlength="5" :disabled="disabled" v-model="mi_title_stitle_img.title" @blur="getVal"></textarea>
    <transition-group name="list-complete" tag="div" mode="in-out" class="flex">
      <section
      ...
      >
        <div class="dingzhi-tupianC-div">
          <img class="defaultImg" :src="item.imgUrl || 'http://style.iambuyer.com/img/rhx-mobile-normal@2x.png'" alt="img" @click.prevent="showChoseImgBtns(item,key)">
          <div class="dingzhi-tupianC-del" v-show="showDel">
            <p class="dingzhi-tupianC-del-btn" @click="DelateThisItems(index)">删除</p>
          </div>
        </div>
        <div class="dingzhi-tupianC-li-b">
          <input
            ...
            @blur="getVal"
          >
          <i :class="{'iconfont icon-wancheng':true, 'on':!item.select}" @click="choose(index)"></i>
        </div>
        <div class="mi_itemConWrap MITEMS">
          <div class="mi_itemConWrap_imgWrap">
            ...
            <!-- <img :src="item.imgUrl || 'http://style.iambuyer.com/img/rhx-mobile-normal@2x.png'" alt="" class="needsclick" @click.prevent="showChoseImgBtns"> -->
          </div>
        </div>
      </section>
    </transition-group>
...
export default {
  ...
  methods: {
    getVal(){
      LS.setItem("imgC1",this.mi_title_stitle_img)
    },
    choose(i) {
      // 多选
      ...
      // 单选
      this.$parent.danx1 = this.mi_title_stitle_img.dataList[i].title;
