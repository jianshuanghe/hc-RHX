        ...
        <ul>
          <li class="dingzhi-base-li" :key="index">
            <input
              ...
              @blur="getVal"
            >
            <input class="dingzhi-cus-ipt" type="text" placeholder="请输入" :disabled="isCus" v-model="item.value" @blur="getVal">
            <input
              :class="{ 'text': !disabled, 'dingzhi-ipt': true}"
              type="text"
              maxlength="4"
...
export default {
  ...
  created(){
    if(LS.getItem("base")){
      this.mi_title_stitle_img = LS.getItem("base");
    }
  },
  data() {
    return {
  ...
  methods: {
    getVal(){
      LS.setItem("base",this.mi_title_stitle_img)
    },
    del(e) {
