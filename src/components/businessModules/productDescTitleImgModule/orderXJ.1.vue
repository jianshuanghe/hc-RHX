<template>
  <div class="PDTIV1" v-if="renderThis">
    ...
    <!-- 选择上传文件bridge -->
    <textarea :class="{'title needsclick order-product-title':true, 'text': editable}" maxlength="5" :disabled="!editable" v-model="mi_title_stitle_img.title"></textarea>
    <textarea :disabled="!editable" :class="{'order-product-ms':true, 'text': editable}" maxlength="20" :style="editTextOutLine" v-model="mi_title_stitle_img.content"></textarea>
    <transition-group name="list-complete" tag="div" mode='in-out'>
      <section
        v-for="(item,index) in mi_title_stitle_img.dataList"
        :key="item" class="MITEMS-WRAP list-complete-item notop"
      >
        ...
        <div class="mi_itemTitleCon">
          <div class="mi_itemTitleConTitleWrap">
            ...
            >{{item.item_title}}</h2> -->
            <!-- <textarea :contenteditable="editable" :class="{'order-product-ms':true, 'text': editable}" :style="editTextOutLine">
        ...
        <div class="mi_itemConWrap MITEMS" v-show="item.imgUrl">
          <div class="mi_itemConWrap_imgWrap">
            <changeImgTips @change-img="changeImg(item,'imgUrl')"
                           @remove-img="removeImg(item,'imgUrl',index)" :choseimgfile="choseimgfile" v-if="show.isChoseImg">
            </changeImgTips>
            <img :src="item.imgUrl || 'http://style.iambuyer.com/img/rhx-mobile-normal@2x.png'" alt="" class="needsclick order-banner-img" @click.prevent="showChoseImgBtns">
          </div>
        </div>
          ...
          dataList: [
            {
              index: 0,
              // item_title: '第一项',
              content: '这里是您的产品具体说明，请详细说明哦。这里是您的产品具体说明，请详细说明哦。',
