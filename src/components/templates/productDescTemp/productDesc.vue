  ...
  <div class="ITC-ROOT">
    ...
    <div class="sortModule" ref="sortModule">
      <div class="sortModuleWrap">
        ...
        <draggable
          class="sortStart"
          v-model="TEMP_ROOT_DATA.module_icon"
          :move="getdata"
          @update="datadragEnd"
          :options="{animation:500}">
          <div v-for="(item,index) in TEMP_ROOT_DATA.module_icon" :key="index" class="sortItem">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      // TEMP_ROOT_DATA(){
      //   return JSON.parse(JSON.stringify(this.GET_TEMP_DATA))
      // },
      selModuleHeight() {
    ...
    data() {
      return {
        ...
        MIcon: [],// 导航栏传过来的模块图标
        MDimgs: [],//模块列表
        TEMP_START_DATA: '',//模板中的原始数据，供追加模块使用
        showModal: false,
        startx: '',
        starty: '',
        isPreview: false,
    ...
    watch: {
      ...
      // GET_TEMP_DATA(){
      //   handler(val, oldval) {
      //     this.TEMP_ROOT_DATA(JSON.parse(JSON.stringify(val)))
      //   },
      //   deep: true
      // }
    },
    methods: {
      // 监听导航中的删除事件，并把删除后的模块带过来
      delThisModule(dtm, icon) {
        this.TEMP_ROOT_DATA.this_temp_all_module_data = dtm
        this.TEMP_ROOT_DATA.module_icon = icon
        // console.log(dtm)
      },
      // 添加模块框显示
      ...
      // push模块
      pushMd: function (i) {
        console.log(this.MDimgs[i].module_path)
        // 隐藏选择模块框
        this.hideAddModule()
        // 要添加的模块名字
        ...
        switch (this.MDimgs[i].module_path) {
          case 'mi_module_product_info':
            console.log('添加产品简介模块...')
            /** 产品简介标题图片 ******************************************************/
              // 复制要添加的模块的内容
            let p_info_top = JSON.parse(JSON.stringify(this.TEMP_START_DATA.this_temp_all_module_data[this.MDimgs[i].module_path]))
            // 要添加的项目名字
            const MD_title_img = this.MDimgs[i].module_path + gethashcode()
            // 更新vuex中的数据
            this.$set(this.TEMP_ROOT_DATA.this_temp_all_module_data, [MD_title_img], p_info_top)
            //更改显示模块图标
            let CopyAddIcon = JSON.parse(JSON.stringify(this.MDimgs[i]))
            CopyAddIcon.module_path = MD_title_img
            this.MIcon.splice(this.MIcon.length - 2, 0, CopyAddIcon)
            this.$set(this.TEMP_ROOT_DATA, 'module_icon', this.MIcon)
            // 定义即将追加的模块数据
            const titleImgItems = Vue.extend({
              ...
              created() {
                ...
                //     this.thisTempData = res.content.context
                //   }
                // })
          ...
          case 'mi_module_product_install':
            ...
            /** 如何安装 **************************************************************/
            let p_install = JSON.parse(JSON.stringify(this.TEMP_ROOT_DATA.this_temp_all_module_data[this.MIcon[i].module_path]))
            // 要添加的项目名字
            const MD_install = this.MIcon[i].module_path + gethashcode()
            // 更新vuex中的数据
            this.$set(this.TEMP_ROOT_DATA.this_temp_all_module_data, [MD_install], p_install)
          ...
          case 'mi_module_product_use':
            ...
            // 复制要添加的模块的内容
            let p_how_use = JSON.parse(JSON.stringify(this.TEMP_ROOT_DATA.this_temp_all_module_data[this.MIcon[i].module_path]))
            // 更新vuex中的模块数据
            this.$set(this.TEMP_ROOT_DATA.this_temp_all_module_data, [MD_how_use], p_how_use)
        ...
        API._getTempDetail(this.$route.query.tempid).then((res) => {
          if (res.ret === API.ERROK) {
            ...
            if (this.GET_TEMP_DATA !== '') {
              ...
              this.TEMP_START_DATA = this.GET_TEMP_DATA
            }
            //mixin赋值给data中的mix
            this.mix = itemsMixin.components
            // this.mix = this.TEMP_ROOT_DATA.module_sort
            // for(let i = 0;i < this.TEMP_ROOT_DATA.module_sort.length;i++){
            //   console.log(this.TEMP_ROOT_DATA.module_sort[i])
            // }
            // 更新vuex后才载入模块
            this.mixin2array()
          } else {
